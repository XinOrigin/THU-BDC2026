...
def main():
	...
	processed[features] = processed[features].replace([np.inf, -np.inf], np.nan).ffill().bfill().fillna(0)
	...
	# 安全校验
	assert len(weights) == 5, f'权重长度应为5，实际为{len(weights)}'
	assert abs(weights.sum() - 1.0) < 1e-6, f'权重总和应为1，实际为{weights.sum()}'
